ymaps.ready(function () {
  ...
  fetch('ao.geojson')
    ...
    .then(geo => {
      geo.features.forEach(f => {
        ...
        const color = AO_COLORS[key] || '#dddddd';
        ...
        if (geom.type === 'Polygon') {
          myMap.geoObjects.add(
            new ymaps.Polygon(convertPolygon(geom.coordinates), { hintContent: nameRaw, balloonContent: nameRaw }, polyOpts)
          );
        } else if (geom.type === 'MultiPolygon') {
          convertMultiPolygonToPolygons(geom.coordinates).forEach(yaCoords => {
            myMap.geoObjects.add(
              new ymaps.Polygon(yaCoords, { hintContent: nameRaw, balloonContent: nameRaw }, polyOpts)
            );
          });
        }
        ...
        if (labelCoord) {
          myMap.geoObjects.add(
            new ymaps.Placemark(labelCoord, { text: nameRaw }, { iconLayout: LabelLayout, hasBalloon: false, hasHint: false, zIndex: 120, zIndexHover: 120 })
          );
        }
      });
    })
  ...
  // ===== 3) Точки (open.json) + инверсия координат =====
  fetch('open.json')
    .then(r => r.json())
    .then(obj => {
