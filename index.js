ymaps.ready(function () {
  ...
  const setLabelSize = px =>
    document.documentElement.style.setProperty('--aoLabelSize', px + 'px');
      ...
      geo.features.forEach(f=>{
        ...
        if (geom.type==='Polygon') {
          myMap.geoObjects.add(
            new ymaps.Polygon(
              convertPolygon(geom.coordinates),
              {hintContent:nameRaw, balloonContent:nameRaw},
              polyOpts
            )
          );
        ...
        const labelCoord =
          AO_LABEL_COORDS[key] || AO_LABEL_COORDS[String(nameRaw).toUpperCase()];
        if (labelCoord){
          myMap.geoObjects.add(
            new ymaps.Placemark(
              labelCoord,
              {text:nameRaw},
              {iconLayout:LabelLayout, hasBalloon:false, hasHint:false, zIndex:120, zIndexHover:120}
            )
          );
        }
      });
